...
/* harmony export */   addPostApi: () => (/* binding */ addPostApi),
...
/* harmony export */   switchLikeApi: () => (/* binding */ switchLikeApi),
...
function switchLikeApi({ token, postId, likedMode }) {
...
function addPostApi({ description, imageUrl, token }) {
  return fetch(postsHost, {
    method: "POST",
    body: JSON.stringify({
...
function renderAddPostPageComponent({ appEl, token }) {
  let imageUrl = "";
...
`;
    ...
    function onAddPostClick() {
      ...
      const descriptionSanitizedHTML = sanitize_html__WEBPACK_IMPORTED_MODULE_5___default()(description, {
        allowedTags: [],
        allowedAttributes: [],
      });
      ...
      if (!imageUrl) {
        ...
        (0,_api_js__WEBPACK_IMPORTED_MODULE_2__.addPostApi)({
          description: descriptionSanitizedHTML,
          imageUrl,
          token,
        })
          .then(() => {
            (0,_index_js__WEBPACK_IMPORTED_MODULE_3__.goToPage)(_routes_js__WEBPACK_IMPORTED_MODULE_4__.POSTS_PAGE);
          })
          .catch((error) => {
            console.warn(error);
...
function renderAuthPageComponent({ appEl, setUser }) {
  ...
  function getSanitizeInput(inputValue) {
    return sanitize_html__WEBPACK_IMPORTED_MODULE_3___default()(inputValue, {
      allowedTags: [],
      allowedAttributes: [],
    });
  }
...
function renderHeaderComponent({ element }) {
  ...
  <div class="page-header">
      ...
      ${
        _index_js__WEBPACK_IMPORTED_MODULE_0__.user
          ? `<button title="${_index_js__WEBPACK_IMPORTED_MODULE_0__.user.name}" class="header-button logout-button">
          <img src="${_index_js__WEBPACK_IMPORTED_MODULE_0__.user.imageUrl}" class="page-header__user-image">
          Выйти</button>`
          : ""
      }  
  </div>
...
__webpack_require__.r(__webpack_exports__);
/* harmony export */ __webpack_require__.d(__webpack_exports__, {
/* harmony export */   postId: () => (/* binding */ postId),
...
let userId; // для страницы постов выбранного юзера
let postId; // для лайков

function renderPostsPageComponent({ appEl, posts, token, user }) {
  const postsHTML = posts
    .map((post) => {
      ...
      const commentTime = (0,date_fns__WEBPACK_IMPORTED_MODULE_4__["default"])(new Date(post.createdAt));
      return `
  ...
  for (let likeButtonEl of document.querySelectorAll(".like-button")) {
    ...
    if (loginUserId !== null) {
      likeButtonEl.addEventListener("click", () => {
        ...
        (0,_api_js__WEBPACK_IMPORTED_MODULE_3__.switchLikeApi)({ token, postId, likedMode })
          .then((postFromApi) => {
  ...
  for (const deleteButtonEl of document.querySelectorAll(".delete-button")) {
    deleteButtonEl.addEventListener("click", () => {
      ...
      if (confirmation) {
        (0,_api_js__WEBPACK_IMPORTED_MODULE_3__.deletePostApi)({ token, postId })
          .then(() => {
...
const goToPage = (newPage, data) => {
  if (
    ...
    if (newPage === _routes_js__WEBPACK_IMPORTED_MODULE_3__.USER_POSTS_PAGE) {
      ...
      return (0,_api_js__WEBPACK_IMPORTED_MODULE_0__.getUserPosts)({ token: getToken(), userId: _components_posts_page_component_js__WEBPACK_IMPORTED_MODULE_4__.userId })
        .then((newPosts) => {
...
const renderApp = () => {
  ...
  if (page === _routes_js__WEBPACK_IMPORTED_MODULE_3__.USER_POSTS_PAGE) {
    // DONE: реализовать страницу фотографий выбранного пользователя
    return (0,_components_posts_page_component_js__WEBPACK_IMPORTED_MODULE_4__.renderPostsPageComponent)({
      appEl,
      posts,
      token: getToken(),
